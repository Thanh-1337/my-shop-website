...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (themeToggle) {
    ...
    themeToggle.addEventListener('click', () => {
      ...
      showToast(isLight ? 'Đã chuyển sang giao diện Sáng!' : 'Đã chuyển sang giao diện Tối!', isLight ? 'fa-sun' : 'fa-moon');
    });
  }
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener('click', () => {
      const cart = getCart();
      if (cart.length === 0) {
        showToast('Giỏ hàng trống! Hãy lựa chọn sản phẩm trước.', 'fa-exclamation-triangle');
        return;
      }
      
      // Tạo sự kiện đặt hàng thành công
      closeDrawer();
      showToast('Đặt hàng thành công! Đơn hàng đang được xử lý.', 'fa-check-circle');
